...
from utils import calculate_joint_entropy
import numpy as np
import pandas as pd
...
def contrastive_train(network_model, mv_data, mvc_loss, batch_size, lmd, beta, gamma, temperature_l, normalized, epoch, optimizer):
    network_model.train()
    ...
    for batch_idx, (sub_data_views, _) in enumerate(mv_data_loader):
        lbps, dvs, features_list = network_model(sub_data_views)
        loss_list = list()
        ...
        joint_entropy_loss = mvc_loss.joint_entropy_loss(features_list)

        loss = sum(loss_list) + gamma * joint_entropy_loss
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
        total_loss += loss.item()
...
def inference(network_model, mv_data, batch_size):
    ...
    print(f"Total number of samples in inference: {actual_num_samples}")
...
def valid(network_model, mv_data, batch_size):
    ...
    print(f"Number of evaluated samples: {len(labels_vector)}")
